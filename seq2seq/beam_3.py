...
class BeamSearch(object):
    ...
    def prune(self):
        ...
        for _ in range(self.nodes.qsize()):
            node = self.nodes.get()
            temp.put(node)
            if node[2].mark == 'completed' and nodes.qsize() < self.beam_size:
                nodes.put(node)
                if best_finished_score == None:
                    best_finished_score = node[0]
                else:
                    if node[0] < best_finished_score: best_finished_score = node[0]
        self.nodes = temp
        
        for _ in range(self.nodes.qsize()):
            if nodes.qsize() >= self.beam_size: break
            node = self.nodes.get()
            if node[2].mark == 'uncompleted':
                if best_finished_score == None:
                    nodes.put(node)
                elif node[0] <= best_finished_score:
                    nodes.put(node)
        self.nodes = nodes
